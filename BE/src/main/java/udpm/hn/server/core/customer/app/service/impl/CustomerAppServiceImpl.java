...
import udpm.hn.server.core.customer.app.repository.CustomerAppImageRepository;
...
public class CustomerAppServiceImpl implements CustomerAppService {
    ...
    private final CustomerAppImageRepository appImageRepository;
    ...
    public PageableObject<CustomerAppResponse> filter(CustomerAppFilterRequest request) {
        ...
        Pageable pageable = PageRequest.of(request.getPage() > 0 ? request.getPage() - 1 : 0, request.getSize(), sort);
    ...
    public CustomerAppResponse getDetail(String id) {
        ...
        // Images
        response.setAppImages(appImageRepository.findByApp_Id(id).stream()
                .map(i -> i.getUrl())
                .collect(Collectors.toList()));
    ...
    @Override
    @Transactional(readOnly = true)
    public PageableObject<CustomerAppResponse> fullTextSearch(String domainId, String query, int page, int size) {
        Pageable pageable = PageRequest.of(page > 0 ? page - 1 : 0, size);

        Page<App> resultPage = appRepository.fullTextSearch(
                domainId,
                query,
                EntityStatus.ACTIVE.name(),
                pageable);

        List<CustomerAppResponse> responses = resultPage.stream()
                .map(this::toSimpleResponse)
                .collect(Collectors.toList());

        return new PageableObject<>(resultPage, responses);
    }

    @Override
    @Transactional(readOnly = true)
    public List<CustomerAppResponse> getFeaturedApps() {
        List<App> featuredApps = appRepository.findFeaturedApps(EntityStatus.ACTIVE);

        return featuredApps.stream()
                .map(this::toSimpleResponse)
                .collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public PageableObject<CustomerAppResponse> getAppsSorted(String domainId, String sortBy, int page, int size) {
        Pageable pageable = PageRequest.of(page > 0 ? page - 1 : 0, size);
        Page<App> resultPage;

        switch (sortBy.toUpperCase()) {
            case "NEWEST":
                resultPage = appRepository.findByDomainOrderByNewest(domainId, EntityStatus.ACTIVE, pageable);
                break;
            case "POPULAR":
                resultPage = appRepository.findByDomainOrderByPopular(domainId, EntityStatus.ACTIVE, pageable);
                break;
            case "FEATURED":
                resultPage = appRepository.findByDomainOrderByFeatured(domainId, EntityStatus.ACTIVE, pageable);
                break;
            default:
                // Default to newest
                resultPage = appRepository.findByDomainOrderByNewest(domainId, EntityStatus.ACTIVE, pageable);
