...
import udpm.hn.server.core.admin.app.model.response.AdminAppImageResponse;
...
import udpm.hn.server.core.admin.app.model.response.AdminMemberResponse;
...
public class AdminAppServiceImpl implements AdminAppService {
    ...
    private final AdminAppMemberRepository appMemberRepository;
    private final AdminCustomerRepository customerRepository;

    @Override
    @Cacheable(value = "allApps", unless = "#result == null || #result.isEmpty()")
    @Transactional(readOnly = true)
    public List<AdminAppResponse> getAll() {
        return appRepository.findAllWithTechnologies().stream()
                .map(this::toResponse)
                .collect(Collectors.toList());
    }

    @Override
    @Cacheable(value = "apps", key = "#id", unless = "#result == null")
    @Transactional(readOnly = true)
    public AdminAppResponse getDetail(String id) {
        return appRepository.findByIdWithTechnologies(id)
                .map(this::toResponse)
                .orElse(null);
    }
    ...
    public String create(AdminCreateAppRequest request) {
        ...
        if (request.getTechnologyIds() != null && !request.getTechnologyIds().isEmpty()) {
            ...
            savedApp.setTechnologies(new java.util.HashSet<>(techs));
    ...
    @Transactional
    @Caching(evict = {
            @CacheEvict(value = "apps", key = "#id"),
            @CacheEvict(value = "allApps", allEntries = true),
            @CacheEvict(value = "customerApps", allEntries = true)
    })
    ...
    @Override
    @Cacheable(value = "userSearch", key = "#query + '_' + #page + '_' + #size", unless = "#result == null")
    public udpm.hn.server.core.common.base.PageableObject<udpm.hn.server.core.admin.app.model.response.AdminUserResponse> searchUsers(
            String query, int page, int size) {
        org.springframework.data.domain.Pageable pageable = org.springframework.data.domain.PageRequest.of(page - 1,
                size);
        org.springframework.data.domain.Page<Customer> customerPage = customerRepository.search(query, pageable);
        List<udpm.hn.server.core.admin.app.model.response.AdminUserResponse> responses = customerPage.stream()
                .map(c -> new udpm.hn.server.core.admin.app.model.response.AdminUserResponse(c.getId(), c.getFullName(),
                        c.getEmail(), "UNKNOWN"))
                .collect(Collectors.toList());
        return new udpm.hn.server.core.common.base.PageableObject<>(customerPage, responses);
    }

    @Override
    public Boolean addMember(String id, AdminAddMemberRequest request) {
        Optional<App> appBox = appRepository.findById(id);
        Optional<Customer> customerBox = customerRepository.findById(request.getCustomerId());

        if (appBox.isPresent() && customerBox.isPresent()) {
            // Check if exists
            Optional<AppMember> existing = appMemberRepository.findByAppIdAndCustomerId(id, request.getCustomerId());
            if (existing.isPresent()) {
                return false; // Already exists
            }

            AppMember member = new AppMember();
            member.setApp(appBox.get());
            member.setCustomer(customerBox.get());
            member.setRole(request.getRole()); // LEADER or MEMBER
    ...
    @Override
    public Boolean removeMember(String id, String memberId) {
        Optional<AppMember> memberBox = appMemberRepository.findById(memberId);
        if (memberBox.isPresent() && memberBox.get().getApp().getId().equals(id)) {
            appMemberRepository.delete(memberBox.get());
            return true;
        }
        return false;
    }
    ...
    @Override
    public List<AdminMemberResponse> getMembers(String id) {
        return appMemberRepository.findAllByAppId(id).stream()
                .map(m -> new AdminMemberResponse(
                        m.getId(),
                        m.getCustomer().getId(),
                        m.getCustomer().getFullName(),
                        m.getCustomer().getEmail(),
                        m.getRole()))
                .collect(Collectors.toList());
    }

    @Override
    public List<String> uploadGallery(String id, List<MultipartFile> files) {
        Optional<App> appBox = appRepository.findById(id);
        if (appBox.isEmpty())
            return List.of();
        ...
        return files.stream().map(file -> {
            String fakeUrl = "https://placeholder.com/gallery/" + file.getOriginalFilename(); // REPLACE WITH REAL
                                                                                              // STORAGE
            AppImage img = new AppImage();
            img.setApp(app);
            img.setUrl(fakeUrl);
            img.setIsMain(false);
            appImageRepository.save(img);
            return fakeUrl;
        }).collect(Collectors.toList());
    }
    ...
    public Boolean deleteImage(String imageId) {
        if (appImageRepository.existsById(imageId)) {
    ...
    private AdminAppResponse toResponse(App app) {
        ...
        if (app.getTechnologies() != null) {
            ...
            response.setTechnologyIds(
                    app.getTechnologies().stream().map(Technology::getId).collect(Collectors.toList()));
        }

        List<AppImage> images = appImageRepository.findAllByAppId(app.getId());
        response.setImages(images.stream()
                .map(img -> new AdminAppImageResponse(img.getId(), img.getUrl(), img.getIsMain()))
                .collect(Collectors.toList()));
