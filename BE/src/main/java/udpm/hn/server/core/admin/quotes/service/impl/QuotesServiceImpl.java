...
import udpm.hn.server.core.admin.quotes.model.request.QuotesItemRequest;
...
import udpm.hn.server.core.admin.quotes.model.response.QuotesItemResponse;
import udpm.hn.server.core.admin.quotes.model.response.QuotesResponse;
import udpm.hn.server.core.admin.quotes.model.response.QuotesSummaryResponse;
import udpm.hn.server.core.admin.quotes.repository.QuotesManageRepository;
import udpm.hn.server.core.admin.quotes.service.QuotesService;
...
public class QuotesServiceImpl implements QuotesService {
    ...
    private final QuotesManageRepository orderRepository;
    private final ProductRepository productRepository;

    @Override
    public ResponseObject<?> createOrder(QuotesCreateRequest request) {
        try {
            ...
            for (QuotesItemRequest itemRequest : request.getItems()) {
            ...
            QuotesResponse response = mapToOrderResponse(savedOrder);
    ...
    @Override
    public ResponseObject<?> updateOrder(String id, QuotesUpdateRequest request) {
        try {
        ...
        log.info("Order updated successfully with ID: {}", updatedOrder.getId());
            ...
            QuotesResponse response = mapToOrderResponse(updatedOrder);
    ...
    public ResponseObject<?> getOrderById(String id) {
        try {
            ...
            QuotesResponse response = mapToOrderResponse(order);
    ...
    public ResponseObject<?> getAllOrders(String search, EntityStatus status, int page, int size) {
        try {
            ...
            Page<QuotesSummaryResponse> response = orders.map(this::mapToOrderSummaryResponse);
    ...
    @Override
    public ResponseObject<?> updateOrderStatus(String id, QuotesStatusUpdateRequest request) {
        try {
    ...
    private QuoteItem createQuoteItem(QuotesItemRequest request, QuoteRequest order) {
    ...
    private QuotesResponse mapToOrderResponse(QuoteRequest order) {
        List<QuotesItemResponse> itemResponses = order.getItems().stream()
    ...
    private QuotesItemResponse mapToOrderItemResponse(QuoteItem item) {
        BigDecimal total = item.getUnitPrice().multiply(BigDecimal.valueOf(item.getQuantity()));

        return QuotesItemResponse.builder()
                .id(item.getId())
    ...
    private QuotesSummaryResponse mapToOrderSummaryResponse(QuoteRequest order) {
        return QuotesSummaryResponse.builder()
                .id(order.getId())
                .orderNumber(generateOrderNumber(order))
                .customerName(order.getCustomerName())
                .customerEmail(order.getCustomerEmail())
