  ...
  <div class="p-6 min-h-screen bg-[#f8f9fa] dark:bg-brand-dark-200 transition-colors duration-300">
    ...
    <div class="flex justify-between items-center mb-6">
      <div class="flex items-center gap-2">
        ...
        <span
          class="px-2.5 py-0.5 rounded-full bg-[#f0ead2] dark:bg-brand-coffee/20 text-[#6c584c] dark:text-brand-cream text-xs font-bold border border-[#e6dfc0] dark:border-brand-coffee/30">{{
    ...
    <!-- TABLE CUSTOM -->
    <TableCustom :data="customers" :columns="columns" :loading="isLoading" :total="customers.length" :pageSize="10">
      <!-- 1. Khách hàng -->
      <template #customer="{ record }">
        <div class="flex items-center">
          <img :src="record.picture || 'https://ui-avatars.com/api/?name=' + record.name"
          ...
          <div>
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400">Đăng ký: {{ new
              Date(record.createdDate).toLocaleDateString('vi-VN') }}</div>
          </div>
        </div>
      </template>
      ...
      <template #contact="{ record }">
        <div class="flex items-center gap-1.5 text-sm text-gray-600 dark:text-gray-300">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-[#a98467] dark:text-brand-coffee"
      ...
      <template #status="{ record }">
        <span
          :class="['px-2.5 py-1 rounded-full text-xs font-medium border', record.status === 'ACTIVE' ? 'bg-[#dde5b6]/40 text-[#386641] border-[#adc178]/30 dark:bg-brand-sage/20 dark:text-brand-sage dark:border-brand-sage/30' : 'bg-red-50 text-red-700 border-red-100 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800']">
      ...
      <template #actions="{ record }">
        <div class="text-center">
          <ButtonCustom :color="record.status === 'ACTIVE' ? 'mocha-soft' : 'success'" size="small"
            @click="toggleStatus(record)" class="min-w-[90px]">
