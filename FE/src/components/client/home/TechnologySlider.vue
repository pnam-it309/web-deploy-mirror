<template>
    <div class="py-12 bg-white dark:bg-gray-800/50 border-y border-gray-100 dark:border-gray-800 overflow-hidden transition-colors">
        <div class="container mx-auto px-4 mb-8 text-center">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Công nghệ sử dụng</h2>
            ...
            <div class="marquee-mover flex w-fit" :style="{ '--marquee-duration': duration + 's' }">
                <div v-for="i in 4" :key="i" class="flex items-center gap-8 marquee-part pr-8" :aria-hidden="i > 1">
                    <div v-for="tech in technologies" :key="tech.id + '-' + i"
                        ...
                        class="flex flex-col items-center justify-center p-4 min-w-[120px] h-[120px] bg-gray-50 dark:bg-gray-800/80 rounded-xl hover:bg-white dark:hover:bg-gray-700 hover:shadow-lg transition-all duration-300 border border-gray-100 dark:border-gray-700 group cursor-pointer">
                        ...
                        <span class="text-sm font-medium text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white">{{ tech.name }}</span>
                    </div>
                </div>
            </div>
