...
const processOAuthCallback = async () => {
  try {
    ...
    try {
      ...
      const decodedState = JSON.parse(atob((state as string).replace(/ /g, '+')))
      ...
      // Decode JWT và lấy thông tin user theo chuẩn
...
const redirectBasedOnRole = (roleScreen: string) => {
  return new Promise<void>((resolve) => {
    let redirectTo = { name: ROUTES_CONSTANTS.CUSTOMER.children.HOME.name }
