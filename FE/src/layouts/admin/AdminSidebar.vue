<template>
  <div 
    class="hidden lg:block h-full flex-shrink-0 transition-[width] duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]"
    :class="[sidebarStore.isDesktopExpanded ? 'w-64' : 'w-20']"
  ></div>
  ...
  >
    <div class="h-16 flex items-center justify-between px-4 bg-dark text-white relative transition-all duration-300 flex-shrink-0">
      <div 
      ...
      <button 
        @click="sidebarStore.toggleDesktopSidebar"
    ...
    <div class="flex-1 overflow-y-auto py-6 px-3 space-y-2 custom-scrollbar flex flex-col w-full">
      ...
      >
        <div 
          class="absolute left-0 top-0 bottom-0 w-1 bg-primary transition-all duration-300"
          :class="[isActive(item.path) ? 'opacity-100' : 'opacity-0']"
        ></div>

        <component 
          :is="item.icon" 
          class="flex-shrink-0 w-6 h-6 transition-colors duration-300 ml-2 relative z-10"
          :class="[isActive(item.path) ? 'text-primary' : 'text-gray-400 group-hover:text-dark']"
        />

        <span 
          class="ml-3 whitespace-nowrap transition-all duration-300 z-10"
    ...
    <div class="p-4 border-t border-gray-100 bg-gray-50">
       <div class="flex items-center gap-3 overflow-hidden">
          <div class="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center font-bold text-xs flex-shrink-0">
            AD
          </div>
